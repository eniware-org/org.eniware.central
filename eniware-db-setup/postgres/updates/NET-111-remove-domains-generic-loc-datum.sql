...
ALTER TABLE eniwareagg.agg_stale_loc_datum
  ALTER COLUMN loc_id SET DATA TYPE bigint,
  ALTER COLUMN source_id SET DATA TYPE character varying(64);

ALTER TABLE eniwareagg.agg_loc_messages
  ALTER COLUMN loc_id SET DATA TYPE bigint,
  ALTER COLUMN source_id SET DATA TYPE character varying(64),
  ALTER COLUMN ts SET DATA TYPE timestamp with time zone;
...
ALTER TABLE eniwareagg.aud_loc_datum_hourly
  ALTER COLUMN loc_id SET DATA TYPE bigint,
  ALTER COLUMN source_id SET DATA TYPE character varying(64);
...
CREATE VIEW eniwareagg.da_loc_datum_avail_hourly AS
WITH loctz AS (
...
CREATE VIEW eniwareagg.da_loc_datum_avail_daily AS
WITH loctz AS (
	SELECT l.id AS loc_id, COALESCE(l.time_zone, 'UTC') AS tz
	FROM eniwarenet.sn_loc l
)
...
CREATE VIEW eniwareagg.da_loc_datum_avail_monthly AS
WITH loctz AS (
	SELECT l.id AS loc_id, COALESCE(l.time_zone, 'UTC') AS tz
	FROM eniwarenet.sn_loc l
)
...
DROP FUNCTION eniwaredatum.store_loc_meta(eniwarecommon.ts, eniwarecommon.loc_id, eniwarecommon.source_id, text);
...
DROP FUNCTION eniwaredatum.find_loc_available_sources(eniwarecommon.loc_id, eniwarecommon.ts, eniwarecommon.ts);
CREATE OR REPLACE FUNCTION eniwaredatum.find_loc_available_sources(
    IN loc bigint,
...
DROP FUNCTION eniwaredatum.find_loc_reportable_interval(eniwarecommon.loc_id, eniwarecommon.source_id);
CREATE OR REPLACE FUNCTION eniwaredatum.find_loc_reportable_interval(
	IN loc bigint,
...
BEGIN
	CASE
		WHEN src IS NULL THEN
			SELECT min(ts) FROM eniwaredatum.da_loc_datum WHERE loc_id = loc
			INTO ts_start;
		ELSE
			SELECT min(ts) FROM eniwaredatum.da_loc_datum WHERE loc_id = loc AND source_id = src
			INTO ts_start;
	END CASE;

	CASE
		WHEN src IS NULL THEN
			SELECT max(ts) FROM eniwaredatum.da_loc_datum WHERE loc_id = loc
			INTO ts_end;
		ELSE
			SELECT max(ts) FROM eniwaredatum.da_loc_datum WHERE loc_id = loc AND source_id = src
			INTO ts_end;
	END CASE;
...
DROP FUNCTION eniwaredatum.find_loc_most_recent(eniwarecommon.loc_id, eniwarecommon.source_ids);
CREATE OR REPLACE FUNCTION eniwaredatum.find_loc_most_recent(
	loc bigint,
	sources text[] DEFAULT NULL)
  RETURNS SETOF eniwaredatum.da_loc_datum AS
$BODY$
BEGIN
	IF sources IS NULL OR array_length(sources, 1) < 1 THEN
		RETURN QUERY
		SELECT dd.* FROM eniwaredatum.da_loc_datum dd
		INNER JOIN (
...
DROP FUNCTION eniwaredatum.find_sources_for_loc_meta(bigint[],text);
